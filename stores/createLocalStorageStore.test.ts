...
describe('createLocalStorageStore', () => {
  ...
  it('falls back to the default value when stored JSON is corrupt', () => {
    ...
    const store = createLocalStorageStore<{ ok: boolean }>('bad-json', {
      ok: true,
    });

    // get() before any subscribe() call returns the in-memory cache
    // (the default), since hydration from localStorage only happens on
    // first subscribe - this mirrors the store's real SSR-safe behavior.
