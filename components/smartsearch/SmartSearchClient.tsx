      ...
      <Card>
        ...
        >
          <input
            ...
            className="flex-1 rounded-lg border border-slate-300 dark:border-slate-600 bg-white dark:bg-slate-900 px-3 py-2 text-sm text-slate-900 dark:text-slate-100 focus:border-orange-400 focus:outline-none"
          />
          <button
            type="submit"
            disabled={loading}
        ...
        <div className="mt-3 flex flex-wrap gap-2">
          {EXAMPLES.map((example) => (
            <button
              ...
              className="rounded-full bg-slate-100 dark:bg-slate-800 px-3 py-1 text-xs text-slate-600 dark:text-slate-300 hover:bg-slate-200 dark:hover:bg-slate-700"
            >
              {example}
            </button>
          ))}
      ...
      {result && !result.recognized && (
        <Card>
          <p className="text-sm text-slate-600 dark:text-slate-300">
            Sorry, I couldn&apos;t understand that query. Try one of the examples above, or a
            similar phrasing.
          </p>
        </Card>
      )}
      ...
      {result && result.recognized && (
        <Card>
          <div className="mb-3 flex items-center gap-2 text-sm font-medium text-slate-500 dark:text-slate-400">
            ...
            {result.interpretedAs ?? 'Results'} &middot; {result.matchCount.toLocaleString()} match
