...
/**
 * Fetches lazily on mount, same reasoning as TrainIntelligenceCard: this is
 * enrichment on top of the core station page, not something that should
 * block or break it if the heavier network computation is slow or fails.
 */
export default function StationIntelligenceCard({ stationCode }: Props) {
  ...
  useEffect(() => {
    ...
    getStationIntelligence(stationCode)
      ...
      .catch((err) => {
        if (cancelled) return;
        setError(err instanceof ApiError ? err.message : 'Could not load station insights.');
      })
      .finally(() => {
  ...
  const stats: { label: string; value: string; icon: typeof Network }[] = [
    {
      label: 'Network rank',
      value: data.networkRank != null ? `#${data.networkRank} of ${data.totalStationsInNetwork}` : 'N/A',
      icon: Award,
    },
    { label: 'Connectivity', value: `${data.connectivityScore}/100`, icon: Network },
    { label: 'Direct links', value: `${data.degree}`, icon: Share2 },
    {
      label: 'Avg speed through',
      value: data.averageTrainSpeedKmh != null ? `${data.averageTrainSpeedKmh} km/h` : 'N/A',
      icon: Gauge,
    },
  ];
  ...
  return (
    <section className="mt-6 rounded-xl border border-slate-200 dark:border-slate-700 bg-white dark:bg-slate-900 px-5 py-4 shadow-sm">
      ...
      <p className="mt-1 text-xs text-slate-500 dark:text-slate-400">
        Computed from every train&apos;s schedule and RailLens&apos;s station network graph -
        free, no external API or live data involved.
      </p>
