...
export default function JourneySearchForm({
  ...
  useEffect(() => {
    async function initialize() {
      ...
      if (
        lastSyncedRef.current.from === initialFrom &&
        lastSyncedRef.current.to === initialTo
      ) {
        return;
      }
      ...
      <div className="flex flex-col gap-3 lg:flex-row lg:items-end">
        ...
        <div className="flex justify-center lg:pb-1">
          <button
            ...
            className="inline-flex h-10 w-10 items-center justify-center rounded-md border border-slate-300 bg-white text-slate-600 transition hover:border-orange-300 hover:bg-orange-50 hover:text-orange-600 disabled:cursor-not-allowed disabled:opacity-50 dark:border-slate-600 dark:bg-slate-900 dark:text-slate-300"
          >
      ...
      <div className="mt-4 flex w-full items-center gap-2 md:w-auto">
        <button
          ...
          className={`inline-flex h-10 w-10 shrink-0 items-center justify-center rounded-md border transition ${
            !from || !to
              ...
              : routeIsFavorite
                ? 'border-orange-300 bg-orange-50 text-orange-600 dark:border-orange-500/50 dark:bg-orange-500/15'
                : 'border-slate-300 text-slate-500 hover:border-orange-300 hover:text-orange-600 dark:border-slate-600 dark:text-slate-400'
          }`}
        >
          <Heart
            className="h-4 w-4"
            aria-hidden="true"
            fill={routeIsFavorite ? 'currentColor' : 'none'}
          />
        </button>
        ...
        <button
          ...
          className={`inline-flex h-10 items-center justify-center rounded-md px-5 text-sm font-medium transition ${
            !from || !to
              ? 'cursor-not-allowed bg-slate-300 text-white dark:bg-slate-600'
