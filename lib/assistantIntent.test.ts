...
describe('resolveIntent', () => {
  ...
  it('recognizes achievements requests', () => {
    expect(resolveIntent('open achievements')).toEqual({
      type: 'achievements',
    });
  });
  it('recognizes railway network requests', () => {
    expect(resolveIntent('show the railway network')).toEqual({
      type: 'network',
    });
  });
  ...
  it('recognizes developers page requests', () => {
    expect(resolveIntent('show developer docs')).toEqual({
      type: 'developers',
    });
  });
  it('recognizes saved journeys requests', () => {
    expect(resolveIntent('show saved journeys')).toEqual({
      type: 'savedJourneys',
    });
  });
  it('still treats a bare 2-5 letter word as a station code, not a new intent (pre-existing trade-off)', () => {
    expect(resolveIntent('stats')).toEqual({
      type: 'station',
      stationCode: 'STATS',
    });
    expect(resolveIntent('admin')).toEqual({
      type: 'station',
      stationCode: 'ADMIN',
    });
  });
  ...
  it('resolves each help: sub-topic to its matching help text', () => {
    ...
    expect(
      buildAssistantResponse(
        { type: 'unknown', query: 'help:stations' },
        [],
        []
      ).message
