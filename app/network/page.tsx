...
export default async function NetworkPage() {
  ...
  return (
    <div className="bg-slate-50 py-6 dark:bg-slate-800">
      <Container>
        <Breadcrumb
          items={[{ label: 'Home', href: '/' }, { label: 'Railway Network' }]}
        />
        ...
        <div className="mb-6">
          ...
          <p className="mt-2 text-gray-600 dark:text-slate-300">
            Every station and train in the dataset, treated as a graph - built
            and analyzed entirely from the schedule data, no external or live
            sources.
          </p>
        </div>
        ...
        {stats ? (
          ...
          <ErrorState
            message={statsError ?? 'Failed to load network statistics.'}
          />
        )}
      </Container>
    </div>
